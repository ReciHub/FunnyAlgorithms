def letter_shift(letter: str, shift: int) -> str:
    ...
    alphabet = 'abcdefghijklmnopqrstuvwxyzàáãâéêóôõíúçABCDEFGHIJKLMNOPQRSTUVWYZÀÁÃÂÉÊÓÕÍÚÇ'
...
def caesar_decypher(str_user : str, shift: int = 3) -> str:
    """Function that shifts a string by a certain
    shift that is also given, then returns the shifted
    string from the brazilian portuguese character set
...
def run() -> None:
    ...
    if user_choice == 0:
        ...
        user_input = input("Please provide the message that will be decyphered: ")
        cyphered_msg = caesar_decypher(user_input)
        print(f'Your message decyphered by the caeser cypher is: {cyphered_msg}')
    elif user_choice == 3:
        user_input = input("Please provide the message that will be decyphered: ")
        user_shift = int(input("Please provide the shift for decypher: "))
        cyphered_msg = caesar_decypher(user_input, user_shift)
        print(f'Your message decyphered by the caeser cypher is: {cyphered_msg}')
    else:
