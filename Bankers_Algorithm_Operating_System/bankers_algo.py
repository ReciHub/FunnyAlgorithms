...
def bankers(n, Allocation, Max, Need):
    ...
    # Seq in which P execute
    Sequence = []

    # Calculation of Process executed with its time frame
    for i in range(n):
        for j in range(n):
            if(Finish[j] == 0):
    ...
    # Defining flag to check the deadlock condition
    tag = 0
    for i in range(n):
        if(Finish[i] == 0):
            tag = 1
...
if __name__ == "__main__":
    ...
    # Defining the List of Allocation, Max and Need
    Allocation = []
    Max = []
    Need = []
    ...
    runtag = bankers(n, Allocation, Max, Need)

    if(runtag == 0):
        print("Granted!")
        # print("The Process Sequence: ") print(Sequence)
    else:
